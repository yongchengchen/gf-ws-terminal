...
func (sws *LogicSshWsSession) receiveWsMsg(exitCh chan bool) {
	...
	for {
		...
		default:
			...
			switch msgObj.Type {
			case wsMsgHeartbeat:
				logrus.Println("heart beat 321")
				_, err := sws.session.SendRequest("date", false, []byte(""))
				if err != nil {
					logrus.WithError(err).Error("heart beat fail")
				}
			...
			case wsMsgResize:
				...
				logrus.Println("resize")
				logrus.Println(msgObj.Cols)
				logrus.Println(msgObj.Rows)
			case wsMsgCmd:
